...
- **IO处理机（通道处理机）**
  ...
  >    \text{字节多路通道极限流量：}
  >    \frac{1}{T_S+T_D} \\
  >    \text{其中} T_S \text{为设备选择时间，} T_D \text{为设备传输一个字节时间} \\
  >    \text{实际最大流量：} \sum_{n=k}v_n ~~~~~ \text{各设备速率之和}
  >    $$
  >
  > 2. $$
  >    \text{数组多路极限流量:} \frac{K}{T_S+KT_D}~~~~~~~\text{其中}K\text{为设备一次传输字节} \\
  >    \text{实际最大流量：}Max(v_i) ~~~~~ \text{最大设备速率}
  >    $$
  >
  > 3. $$
  >    \text{选择通道极限流量：} \frac{N}{T_S+NT_D} \text{(一台设备把N个字节全部传输)} \\
  >    \text{实际最大流量：} Max(v_i) ~~~~~ \text{最大设备速率}
  >    $$
