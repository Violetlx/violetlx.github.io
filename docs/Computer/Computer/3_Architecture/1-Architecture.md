...
$$
\text{计算机系统} = \frac {\text{硬件}} {\text{固件（如} BIOS \text{系统）}} + \text{软件}
$$
...
其中：
- $f_{\text{new}}$：性能可改进比（系统在该功能上花费的时间占比），满足 $0 \le f_{\text{new}} \le 1$
- $r_{\text{new}}$：部件加速比（改进前时间 / 改进后时间），满足 $r_{\text{new}} > 1$
- $S_p$：系统加速比（原执行时间 / 改进后执行时间）
