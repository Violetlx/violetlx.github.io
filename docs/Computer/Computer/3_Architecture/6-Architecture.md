...
3. **移数函数PM2I**
   ...
   > \end{array}
   > \right. \\
   > N \text{表示处理单元输入输出端点数，} x \text{表示连接端点}(0\le x \le N-1),
   > \\i \text{表示函数下标} (0\le i \le n-1) \text{，其中} n = log_2N \text{表示}n\text{与}N\text{的关系}\\
   > PM2\text{函数为}2n-1 \text{个}
   > $$
   >
   > ---
   >
...
4. **混铣交换**(置换函数)
   ...
   >$$
   >shuffle:o(x_{n-1},x_{n-2},...x_1,x_0) = x_{n-2}x_{n-3}....x_1x_0x_{n-1} \\ (\text{把输入端的二进制编号循环向左移动一位})  \\
   >shuffle_k : o_{(k)} (x_{n-1}...x_k | x_{k-1},x_{k-2}...,x_0) = x_{n-1}...x_k | x_{x-2}...x_0x_{k-1}  \\ (\text{输入端二进制编号中从右开始第k位向左循环一位}) \\
   >shuffle_k : o^{(k)} (x_{n-1}...x_k | x_{k-1},x_{k-2}...,x_0) = x_{n-1}...x_k | x_{x-2}...x_0x_{k-1}  \\ (\text{输入端二进制编号中从左开始第k位向左循环一位})
   >$$
   >![images/6-Architecture/image-20220905101837878.png](images/6-Architecture/image-20220905101837878.png) 
   >
...
- 交叉开关
  ...
  > 1. 直连 （0表示直连）
  > 2. 交换(出入端互换) （1表示交换）
  > 3. 上播（$i_\text{入}$ 连$i_\text{出}$，$j_\text{出}$ 连$j_\text{入}$）
  > 4. 下播（$j_\text{入}$ 连$i_\text{出}$，$j_\text{出}$ 连$i_\text{入}$）
  >
...
- 多级混洗交换网络(采用单元控制，四功能** **，**级之间采用混洗网络拓扑**)
