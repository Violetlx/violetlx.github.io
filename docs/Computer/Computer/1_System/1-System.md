...
1. CPU利用率： $\text{利用率} = \frac{\text{忙碌时间}}{\text{总时间}}$
...
- 抢占式：在执行过程中进行优先级调度
  ...
  1. 时间片轮转
  2. 高响应比：$ \text{响应比} = \frac{\text{作业等待时间} + \text{估计运行时间}}{\text{估计运行时间}} $
  3. 多级队列调度及反馈
...
1. 二元序列表示 $<P_1,r_2> \text{表示} P_1 \text{正在申请} r_2 \text{资源，}<r_1,P_1) \text{表示} P_1 \text{占有} r_1 \text{资源}$ 
