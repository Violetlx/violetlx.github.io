...
- 位错（比特位出错）
  ...
  1. **检错编码**（积偶校验，循环冗余码CRC）
     ...
     **CRC循环冗余码：**  
     ...
     $$
     FCS \text{位数为多项式的阶（多项式} N \text{位，其阶为}N-1 \text{）；若多项式采用 } X^n + X^{n-1} + \cdots \text{ 描述，转换规则为存在幂次项补1，不存在补0}
     $$
...
- 停止等待协议：单帧等待确认回复，效率低
  ...
  > 信道利用率的计算公式为：
  > $$
  > \text{信道利用率} = \frac{T_D}{T_D + RTT + T_A} = \frac{L}{C \cdot T}
  > $$
  > 
  > 其中：
  > - $T_D$：发送方占用信道的时间
...
- 滑动窗口协议
  ...
  1. GBN（后退N帧协议，发送窗口大于1，接收端口等于1）
     ...
     >
     >   ![images/4-Networks/image-20230305212011960.png](images/4-Networks/image-20230305212011960.png)
     >
