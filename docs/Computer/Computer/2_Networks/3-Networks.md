...
一个码元携带 $n$ bit（位）信息量，则 $M$ 波特（Baud）的码元对应的信息传输率为：

$$
V = M \times n
$$

表示 $N$ 进制的码元需要使用 $\log_2 N$ 比特位表示。
 **信号失真因素：**
...
$$
\text{极限码元传输率} = 2Wlog_2V ~~ (b/s)   \\
  W \text{为信道频率，}V \text{表示离散信号种类}
$$
...
- 信噪比 = $\frac{\text{信号平均功率}}{\text{噪声平均功率}} = \frac{S}{N}$  单位分贝 dB   单位转换  $1 db = 10log_{10} \frac{S}{N}$
- 在带宽受限且有噪声的信道中，求极限比特率大小
- $V_{max} = W log_2(1+ \frac{S}{N}) ~~~(b/s)~~~W \text{表示信道频率}(Hz)$
...
- **模拟数据编码为数字信号** （PCM）
  ...
  1. 抽样：对模拟信号周期性扫描，把时间上连续信号变成离散信号 $f_{\text{采样}}\ge 2f_{\text{模拟最高频率}}$
